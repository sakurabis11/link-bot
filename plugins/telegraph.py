...
async def telegraph_upload(bot, update):
    ...
    download_text = await update.reply_text(text="<code>Downloading To My Server ...</code>", disable_web_page_preview=True)

    # Download the media from the replied message
    media = await update.reply_to_message.download()

    # Update the message to indicate that the file has been downloaded
    await download_text.edit_text(text="<code>Downloading Completed. Now I am Uploading to telegra.ph Link ...</code>", disable_web_page_preview=True)

    # Get the upload speed
    async with aiohttp.ClientSession() as session:
        async with session.get('https://speed.cloudflare.com/api/v4/get') as response:
            if response.status == 200:
                upload_speed = await response.json()
                upload_speed = upload_speed['result']['upload']['bps'] / 1000000
    ...
    try:
        ...
        print(error)
        await download_text.edit_text(text=f"Error :- {error}", disable_web_page_preview=True)
        return
