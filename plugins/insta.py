...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="@mrtgcoderbot")
        else:
            dump_file=await message.reply_video(url,caption="@mrtgcoderbot")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="@mrtgcoderbot")
                   else:
                       dump_file=await message.reply_video(content_value, caption="@mrtgcoderbot")
               except:
                   ...
                   dump_file=await message.reply_video(downfile,caption="@mrtgcoderbot") 
            elif "/p/" in url:
                  meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                  if meta_tag.ok:
                     res=meta_tag.json()
              ...
              #    await message.reply(meta)
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="@mrtgcoderbot")
                  except:
          ...
          #     await message.reply(tracemsg)
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("Use new features")
