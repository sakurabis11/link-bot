...
@Client.on_message(filters.command("inst") & filters.user(6890139663))
async def install_packages(client, message):
    package_names = message.text.split()[1:]

    if not package_names:
        await message.reply_text("Pʟᴇᴀsᴇ sᴘᴇᴄɪғʏ ᴘᴀᴄᴋᴀɢᴇs ᴛᴏ ɪɴsᴛᴀʟʟ (e.g:- <code>/inst enchant</code>)")
        return
...
@Client.on_message(filters.command("run") & filters.user(6890139663))
async def executor(client, message):
    try:
        code = message.text.split(" ", 1)[1]
    except:
        return await message.reply('Cᴏᴍᴍᴀɴᴅ Iɴᴄᴏᴍᴘʟᴇᴛᴇ!\ɴUsᴀɢᴇ: /run python_code')
